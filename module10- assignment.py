...
# Fill missing loyalty_status with "Bronze"
...
df_typed["total_purchases"] = df_typed["total_purchases"].fillna(0).astype(int)
df_typed["age"] = df_typed["age"].fillna(int(df_typed["age"].median())).astype(int)
...
df_no_duplicates["purchase_frequency_category"] = df_no_duplicates["total_purchases"].apply(purchase_frequency)
...
# Keeping all columns because they are useful for analysis-ready CRM data
df_final = df_renamed.drop(columns=["state"], errors="ignore")
...
print("- Data Type Issues: date columns had mixed formats, total_spent had commas/$ signs, and some numeric fields needed conversion")
